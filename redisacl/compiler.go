// Package redisacl compiles service-local Redis execution identities.
//
// It is not a distributed identity provider and does not authenticate external
// principals. Consumers authenticate and authorize callers elsewhere, then use
// this package to constrain the Redis connection that executes approved work.
package redisacl
...
// Compiler materializes local Redis ACL rules. It must never be treated as the
// source of global identity or organization authorization policy.
...
func New(config Config) (*ACLCompiler, error) {
	...
	return &ACLCompiler{config: config}, nil
}

func (p *ACLCompiler) Name() string {
	if p == nil {
		return ""
	}
...
func (p *ACLCompiler) AdminUser() string {
	if p == nil {
		return ""
	}
...
func (p *ACLCompiler) Scope(tenant, username string) (Scope, error) {
	if p == nil {
		return Scope{}, errors.New("redis ACL compiler is nil")
	}
...
func (p *ACLCompiler) Rules(spec UserSpec) ([]string, error) {
	if p == nil {
		return nil, errors.New("redis ACL compiler is nil")
	}
